...
const NavVisualizer: React.FC<NavVisualizerProps> = ({ total, current, className }) => {
    ...
    return (
        <div className={cn("w-full aspect-square relative overflow-hidden", styles.scannerContainer, className)}>
            {/* CRT Effects */}
            <div className={styles.scanlines}></div>
            <div className={styles.staticOverlay}></div>
            <div className={styles.refreshLine}></div>

            {/* Content Layer */}
            <div className={cn("w-full h-full relative")}>

                {/* Grid Overlay - Blue/Purple Theme */}
                <div className="absolute inset-0 z-10 grid grid-cols-4 grid-rows-4 pointer-events-none opacity-20">
                    <div className="border-r border-b border-cyan-500/30"></div>
                    <div className="border-b border-cyan-500/30"></div>
                    <div className="border-r border-b border-cyan-500/30"></div>
                    <div className="border-b border-cyan-500/30"></div>
                ...
                <div className="absolute inset-[15%] border border-cyan-500/10 rounded-full z-0"></div>
                <div className="absolute inset-[35%] border border-violet-500/10 rounded-full z-0"></div>
                <div className="absolute inset-[65%] border border-cyan-500/10 rounded-full z-0"></div>

                {/* Radar Sweep Animation (Blue/Purple) */}
                <div className={cn("absolute inset-0 z-20", styles.radarSweep)}></div>
                ...
                <div className="absolute inset-0 z-30 pointer-events-none">
                    ...
                    >
                        <svg className="w-6 h-6 text-chrome-blue drop-shadow-[0_0_6px_rgba(0,186,255,0.8)]" viewBox="0 0 98 96" xmlns="http://www.w3.org/2000/svg">
                ...
                {/* Text Overlay */}
                <div className="absolute bottom-1 right-2 text-[8px] font-mono text-cyan-400/70 z-30">
                    TGT_{String(current + 1).padStart(2, '0')}
                </div>
                <div className="absolute top-1 left-2 text-[8px] font-mono text-violet-400/70 z-30 flex flex-col gap-0.5 leading-none">
                    <span>SYS_SCAN</span>
                    <span className="text-[6px] opacity-50">MODE: AUTO</span>
                </div>
            </div>
        </div>
    );
